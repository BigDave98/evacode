...
module.exports = {
  supabase,  // Export the client directly
    const createUserInDatabase = async (first_name, last_name, email, hashedPassword, phone_number = null, photo = null) => {
      let { data: authData, error: authError } = await supabase.auth.admin.createUser({
        email: email,
        password: hashedPassword || crypto.randomBytes(16).toString('hex'),
        email_confirm: true,
        user_metadata: {
